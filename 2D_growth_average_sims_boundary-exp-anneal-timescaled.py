...
# foldername='cell auto results/log-time-early-test/include-growth/scaled-t/starting-config-comp/{}nucs_{}size_{}growth-scaled-time-{:.2E}rate-markov-poisson_{}anneal_hard/tau{:.2E}-exp-anneal_bound{:.1E}-{:.1E}-ave{}'.format(nCen,nSize,nSteps_growth,rate,nSteps_anneal,anneal_tau,boundary_thres_start,boundary_thres_final,num_iters)
foldername=f'cell auto results\\paper_standard\\tau{anneal_tau:.2E}'
...
for rep in tqdm(range(num_iters)):
    ...
    border_anneal_times=rng.exponential(anneal_tau,border_inds.shape)
...
np.savez(os.path.join(foldername,f'expon_multi_average_arrays_w-area{num_prev_runs}_iter{num_iters}.npz'),corr_funcs=corr_funcs,intensity_funcs=intensity_funcs,ROI_mins=ROI_mins,ROI_maxs=ROI_maxs,new_phase_area=new_phase_area,timeSteps=timeSteps,percents_anneal=percents_anneal)